...
def download_comfyui_image(image_meta: Dict[str, Any]) -> str | None:
    ...
    try:
            ...
            with output_path.open("wb") as output_file:
                for chunk in response.iter_content(chunk_size=1024 * 64):
                    ...
                    if total_size > max_size:
                        ...
                        logger.warning("Downloaded image exceeded max size: %s", safe_name)
                        return None
        ...
        logger.error("Failed to write ComfyUI image %s: %s", filename, exc)
        return None
...
def generate_image():
    ...
    try:
        ...
        logger.error("ComfyUI response was not JSON: %s", response.text)
        return jsonify({"error": "Invalid response from ComfyUI"}), 502

    prompt_id = data.get("prompt_id")
    if not prompt_id:
        logger.error("ComfyUI response missing prompt_id: %s", data)
